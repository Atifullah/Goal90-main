<template>
  <div class="input-number">
    ...
    <input type="number" :min="min" :max="max" :step="step" :value="num" @keydown.up.prevent="increment"
      @keydown.down.prevent="decrement" @mousewheel.prevent @focus="showFocus" @blur="hideFocus" @input="updateInput" />
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    increment() {
      ...
      this._updateValue(newVal > 0 ? newVal : 0)
    },
    decrement() {
      let newVal = np.plus(this.getCurrentValue * 1, -1 * this.step)
      const stepary = this.step.toString().split('.')
