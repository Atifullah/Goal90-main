<template>
  <div class='header media-screen'>
    <nuxt-link class="fl" tag="div" to="/">
      <img class='logo' src="../../static/images/b_orginal.png" alt="" width="auto" height="10">
    </nuxt-link>
    <div class="head-menu fl">
      <div class="new-money">
        <div class="money-type"
          ...
          <span class="select_collect" :class="{'active':iscollect}" @click.stop="collect(copeDailyDetail)"></span>
          <span class="type-weight">{{symblefrom}}</span>
          <span class="type-tip">/</span>
          <span class="type-normal">{{$t('contract.swap')}}</span>
        </div>
        <div class="new-money-list">
          <div class="new-money-list-content">
            ...
            <p class="new-money-num" v-if="copeDailyDetail.symble">
              <span class="new-money-dot">{{global_get_tofixed_entrust(newmoney.price, dcs.p)}}</span>
              <span
                class="new-money-price">{{getNewMarketClientRate(newmoney.price)}} {{$store.state.hex_client_exchange.active.toUpperCase()}}</span>
            </p>
            <p v-else>
              --
            </p>
          ...
          <div class="new-money-list-content">
            ...
            <p class="new-money-num" v-if="copeDailyDetail.symble">
              <span class="new-money-dot">{{global_get_tofixed_entrust(copeDailyDetail.h, dcs.p)}}</span>
              <span
                class="new-money-price">{{getNewMarketClientRate(copeDailyDetail.h)}} {{$store.state.hex_client_exchange.active.toUpperCase()}}</span>
            </p>
            <p v-else>
              --
            </p>
          ...
          <div class="new-money-list-content">
            ...
            <p class="new-money-num" v-if="copeDailyDetail.symble">
              <span class="new-money-dot">{{global_get_tofixed_entrust(copeDailyDetail.l, dcs.p)}}</span>
              <span
                class="new-money-price">{{getNewMarketClientRate(copeDailyDetail.l)}} {{$store.state.hex_client_exchange.active.toUpperCase()}}</span>
            </p>
            <p v-else>
              --
            </p>
          ...
          <div class="new-money-list-content">
            ...
            <p class="new-money-num" v-if="copeDailyDetail.symble">
              <span class="new-money-dot">{{global_get_tofixed_entrust(copeDailyDetail.v, dcs.a)}}</span>
            </p>
            <p v-else>
              --
            </p>
           ...
           <div class="right-content">
            <div class="select_background">
              ...
              <span class="select_background_night" :class="{'active':background=='night'}" @click="setBackground('night')"></span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="user-info-block set-currency"
        ...
        <div class="set-currency_content"
          ...
          <div class="set-currency_content_exchange clearfix">
             <span class="left" v-for="(item,index) in exchange.infos" :key="index"
                    @click="setExchange(item.name)"
                    :class="{'active':exchange.active==item.name}">{{global_get_uppercase(item.name)}}</span>
          </div>
        </div>
      </div>
      <!--注册登录-->
          ...
          <div class='header-user-info_content'
            ...
            <nuxt-link to="/person/wallet" tag="div" class='ovh estimate'>
              ...
              <p class='client_amount cl_white cion'>
                {{global_get_tofixed_entrust($userassets.totalamountbtc)}} BTC
              </p>
  ...
  export default {
    ...
    methods: {
      getPath(){
        ...
        let sto_name = _arr[1]
        if(sto_name=='deal' || 'margin' || 'contract'){
...
<style lang="scss" scoped>
  ...
  ol.public {
    ...
    li.before:after {
      background: url('../../static/images/an_select_down.png') no-repeat center center;
      content: '';
      display: inline-block;
      vertical-align: middle;
      width: 12px;
    ...
    li.before.cur:after {
      background: url('../../static/images/head_white.png') no-repeat center center;
      content: '';
      display: inline-block;
      vertical-align: middle;
      width: 12px;
