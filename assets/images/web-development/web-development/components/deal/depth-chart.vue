<template>
  <div class="depth-chart" ref="depth-chart">
    <canvas id="depth" ref="canvas" @mousemove="mouseMove"></canvas>
    <canvas id="tip" class="tip" ref="canvastip" width=120 height=40></canvas>
    <canvas id="tiparc" class="tiparc" ref="canvastiparc" width=20 height=20></canvas>
  </div>
</template>
<script>
const _daybg = {
    ...
    getSort: function () {
      ...
      for (let i in buy) {
        ...
        for (let n = 0; n <= i; n++) {
          ...
          price = this.$np.plus(price, buy[n]['p']);
        }
      ...
      for (let i in sell) {
        ...
        for (let n = i; n < sell.length; n++) {
          ...
          price = this.$np.plus(price, sell[n]['p']);
        }
