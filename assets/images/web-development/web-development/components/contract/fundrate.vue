  <template>
    <div class="content_body">
                ...
                <div class="mr-tabs">
                    <ul>
                        <li :class="itype == 1 ? 'active' : ''" @click="ChangeType(1)">
                            <span>{{ $t('contract.realtime') }}</span></li>
                    </ul>
                </div>
            </div>
        ...
        <a-row class="pt-3">
            <a-col :lg="24">
                <a-table :columns="settlementColums" :pagination="false" :data-source="dataDetail" :loading="loading" :rowKey="(record,index ) => index">
                    <div slot="price" slot-scope="text,item">
                        <span>{{ item.indexprice.toFixed(2) }}</span>
                    </div>
                    <div slot="time" slot-scope="text,item">
                        <span>{{item.createtime.replace('T',' ').replace('Z','')}}</span>
                    </div>
                    <div slot="product" slot-scope="text,item">
                        <span>{{ symblename.replace("/YX", "/USDT") }}{{ $t('contract.swap') }}</span>
                    </div>
                    <div slot="rate" slot-scope="text,item">
                        <span>{{ global_get_tofixed(item.fundrate* 100 ,3) }}%</span>
                    </div>
                </a-table>
            </a-col>
        </a-row>
            ...
            this.$store.dispatch('get_getfundrates_infos', { ctype: this.ctype, ctid: this.ctid }).then(({ data }) => {
                if (data) {
                    ...
                    data.reverse().forEach(el => {
                        ...
                        _self.echarts_ydatas.push("" + _self.global_get_local_time(el.t).format('YYYY-MM-DD HH:mm:ss') + "");
                    });
                    this.mcharts();
                } else {
