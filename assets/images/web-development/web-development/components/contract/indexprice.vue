...
<a-card :bordered="false" class="price-card-index mt-3">
        <a-row class="pt-3">
            <a-col :lg="24">
                <a-table :columns="settlementColums" :pagination="true" :data-source="indexPriceData" :rowKey="(record,index ) => index">
                    <div slot="price" slot-scope="text,row">
                        <span>{{ row.price.toFixed(2) }}</span>
                    </div>
                    <div slot="dtime" slot-scope="text,row">
                        <span>{{ row.dtime.replace('T', ' ').replace('Z', '') }}</span>
                    </div>
                    <div slot="weight" slot-scope="text,row">
                        <span>1</span>
                    </div>
                </a-table>
            </a-col>
    ...
    data() {
        return {
            ...
            settlementColums: [
                ...
                scopedSlots: { customRender: 'dtime' },
                width: 120,
                },
                {
            ...
            this.$store.dispatch('quotation_kline_get', klineParam).then(({ data }) => {
                if (data) {
                    ...
                    const bars = data.map(el => {
                        return [
                            "" + _self.global_get_local_time(el.t).format('YYYY-MM-DD HH:mm:ss') + "", //TradingView requires bar time in ms
                            el.o.toFixed(2),
                            el.c.toFixed(2),
                            el.l.toFixed(2),
                            el.h.toFixed(2)
