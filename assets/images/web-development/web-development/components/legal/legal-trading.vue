    ...
    <div class="traded-block">
      <div class="shadow-card pt-4 px-4 pb-4">
        <a-row :gutter="16">
          ...
          <a-col :lg="4">
            <hex-select ref="dirction" :type="'Type'" :order="direction" :market="directiontext" @searchData="searchData"
              @change="change" @clearCurrentState="clearCurrentState" />
          </a-col>
          <a-col :lg="4">
            <hex-select ref="orderstatus" :type="$i18n.t('patch.Status')" :order="orderstatusselect"
              :market="orderstatusselecttext" @searchData="changeStatus" @change="change"
              @clearCurrentState="clearCurrentState" />
          </a-col>
          <a-col :lg="6">
      ...
      <div class="currency-detail px-4 pt-2">
        ...
        <a-card v-for="(item, index) in orderDetail" :key=index :bordered="false" class="currency-detail-card"
          ...
          <a-row class="px-3 curency-detail-bg">
            <a-col :lg="14">
              <p class="mb-0 currency-txt fw-5"><span :class="item.direction == 1 ? 'buy-txt' : 'sell-txt'">{{
                item.direction == 1 ? $t('returntxt.rtxt13') : $t('returntxt.rtxt12') }}</span><a-divider
                  type="vertical" class="mx-2" />{{ global_get_local_time(item.createtime).format('YYYY-MM-DD hh:mm a') }}
              </p>
            </a-col>
          ...
          <a-row :gutter="16" class="px-3 py-4 col-alignment">
            <a-col :lg="3">
              <p class="col-alignment currency-txt fw-5 usdt-txt mb-0"><img :src="getCurrencyIcon(item)" width="20" height="20"
                  class="mr-2" />{{ item.currencyname }}</p>
            </a-col>
            <a-col :lg="4">
              <p class="currency-txt fw-5 usdt-txt mb-0">{{ (item.amount * item.price).toFixed(2) }} {{ item.legaltype }}</p>
            </a-col>
            <a-col :lg="3">
            ...
            <a-col :lg="4">
              <p class="currency-txt fw-5 usdt-txt mb-0 pointer" @click="goCounterParty(item)"><u>{{ item.nikename }}</u></p>
            </a-col>
            <a-col :lg="3">
              <p class="currency-txt fw-5 usdt-txt mb-0">
                <span v-if="item.orderstatus == 0">
                  {{ item.userid == $userinfo.uid ? $t('c2c.selfBuyer') : $t('c2c.buyers') }}
                </span>
                <span v-else-if="item.orderstatus == 4" class="yellow-active">
                  {{ $t('c2c.seller') }}
                </span>
                <span v-else-if="item.orderstatus == 1 || item.orderstatus == 5 || item.orderstatus == 7"
        ...
        <NoData v-if="orderDetail.length == 0" :loading="loading"/>
          ...
          <a-col align="end">
            <div class="text-right" v-if="total > 0">
              <a-pagination background @change="handChange" :page-size="serachOrder.pagesize"
                :current="serachOrder.pageindex" popper-class="paging" layout="prev, pager, next" :total="total">
              </a-pagination>
            </div>
          </a-col>
        </a-row>
...
export default {
  ...
  methods: {
    ...
    gotrade(order) {
      this.$router.push({ path: 'in-process/' + order.id })
    },
    goCounterParty(item) {
    ...
    changeStatus(val) {
      ...
      this.searchParam['orderstatus'] = val.id;
      this.getUserDeal(false);
    },
    ...
    getUserDeal(val, pageIndex) {
      ...
      // console.log(this.typeOfOrder)
      if (orderstatus.length > 0) {
        serachOrder[_params.orderstatus.type] = _params.orderstatus.id;
      }
      else if(this.typeOfOrder==1){
  this.searchParam['orderstatus'] = [0,4,6]
}
else{
  this.searchParam['orderstatus'] = [1,2,3,5,7]
}
      // else {
      //   serachOrder.orderstatus = [];
      //   serachOrder.orderstatus.push(0);
      //   serachOrder.orderstatus.push(4);
      //   serachOrder.orderstatus.push(6);
      // }
