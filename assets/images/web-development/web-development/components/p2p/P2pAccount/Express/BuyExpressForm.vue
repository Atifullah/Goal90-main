        ...
        <a-form-model ref="applyForm" :model="BuyForm" :rules="setValidation.rules" :hide-required-mark="true"
            ...
            <a-row class="mx-4">
                <a-col :lg="24" class="mt-4">
                    <a-form-model-item has-feedback :label="$t('express.exp45')" prop="Sell" :colon="false" :wrapper-col="{
                        xl: 24,
                        lg: 24,
                        md: 24,
                        sm: 24,
                        ...
                        <a-input :placeholder="$t('placeholders.plh28')" class="quantity-input" type="number"
                            v-model="BuyForm.SellAmount" @change="onAmountChange('Sell')">
                            <div slot="suffix">
                                <a-select style="width: 115px" :open="open" v-model="BuyForm.Sell"
                                    @change="onPriceTypeChange" ref="fiatSelect" v-on:select="setOpen(false)"
                                    @dropdownVisibleChange="onOpen" :loading="FormData.Fiats.length == 0">
                ...
                <a-col :lg="24" class="mt-2 pb-5 mb-4">
                    <a-form-model-item has-feedback :label="$t('express.exp46')" prop="Buy" :colon="false" :wrapper-col="{
                        xl: 24,
                        lg: 24,
                        md: 24,
                        sm: 24,
                        ...
                        <a-input :placeholder="$t('placeholders.plh29')" class="quantity-input" type="number"
                            ...
                            <div slot="suffix">
                                <a-select style="width: 115px" @change="onPriceTypeChange" v-model="BuyForm.Buy">
                ...
                <a-col :lg="24" class="mt-4">
                    ...
                    <a-button class="primary-btn fw-5 success-btn mb-3" block @click="handleSubmit"
                        :class="adData != null ? '' : 'disabled-btn'">{{$userinfo.uid?$t('express.exp14'):$t('login.login') }}</a-button>
...
export default {
    ...
    data() {
        return {
            BuyForm: {
                Sell: this.FormData.DefaultIndexOfFiat,
                Buy: this.FormData.DefaultIndexOfCrypto,
                SellAmount: null,
                BuyAmount: null,
            },
            adData: null,
                        ...
                        validator: async (rule, value, callback) => {
                            if (this.BuyForm.BuyAmount === '' || this.BuyForm.BuyAmount == null) {
                                ...
                                if (parseFloat(this.BuyForm.BuyAmount) < parseFloat(this.BuyForm.BuyMin)) {
                                    callback(new Error(`${this.$t('errors.err13')} ${this.BuyForm.BuyMin}`))
                                }
                                else if (parseFloat(this.BuyForm.BuyAmount) > parseFloat(this.BuyForm.BuyMax)) {
                                    callback(new Error(`${this.$t('errors.err2')} ${this.BuyForm.BuyMax}`))
                                }
                                else {
                                    callback()
                                }
    ...
    methods: {
        ...
        async getAdsData() {
            const requestData = {
                currencyid: this.FormData.CryptoCurrencies[this.BuyForm.Buy].id,
                legaltype: this.FormData.Fiats[this.BuyForm.Sell].name,
                pageindex: 1,
                pagesize: 10,
                direction: -1,
                orderstatus: [0]
        ...
        async onPriceTypeChange() {
            ...
            if (this.adData != null) {
                this.EstimatedPriceText = this.$t('express.exp47') +this.adData.currencyname + " ≈ " + this.adData.price + " " +  this.adData.legaltype ;
                this.setMinMax()
            }
            else {
        ...
        setMinMax() {
            if (this.adData != null) {
                ...
                this.BuyForm.BuyMin = this.global_get_tofixed((this.adData.minprice / this.adData.price), this.global_get_decimal(this.adData.legaltype).a)
                this.BuyForm.BuyMax = this.global_get_tofixed((maxpriceCalulated / this.adData.price), this.global_get_decimal(this.adData.legaltype).a)
                this.BuyForm.SellMin = this.global_get_tofixed((this.adData.minprice), this.global_get_decimal(this.adData.currencyname).a)
                this.BuyForm.SellMax = this.global_get_tofixed((maxpriceCalulated), this.global_get_decimal(this.adData.currencyname).a)
            }
        },
        onAmountChange(val) {
            if (this.adData != null) {
                if (val == 'Buy') {
                    this.BuyForm.SellAmount =this.global_get_tofixed((this.BuyForm.BuyAmount * this.adData.price),this.global_get_decimal(this.adData.legaltype).a)
                }
                else {
                    this.BuyForm.BuyAmount =this.global_get_tofixed((this.BuyForm.SellAmount / this.adData.price),this.global_get_decimal(this.adData.currencyname).a)
                }
            }
        },
        ...
        handleSubmit() {
            if(this.$userinfo.uid){
                ...
                if (valid && this.adData != null) {
                    this.BuyForm.BuyObject = this.FormData.CryptoCurrencies[this.BuyForm.Buy]
                    this.BuyForm.SellObject = this.FormData.Fiats[this.BuyForm.Sell]
