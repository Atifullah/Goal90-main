<template>
  <a-table :columns="columns" :pagination="pagination" :data-source="marketdata" :expandIconColumnIndex="7"
    :expandIconAsCell="false" :rowKey="(record, index) => {  return record.id + index }" @change="pageChange">
    <div slot="pair" slot-scope="text,row">
      <div class="pointer"
        @click="goDeal('' + row.symble + '', row.itype == 1 ? 'contract' : (row.itype == -1) ? 'margin' : 'spot')">
        <p class="mb-0 col-alignment">
         
          <img :src="row.logo" style="width:24px; height:24px;" class="mr-2" />
          <span class="symbolFrom">{{ getSymble(row.symble).from }}</span><span v-if="row.itype != '1'"
            class="symbolTo">/{{ getSymble(row.symble).to }}</span><span v-else
            class="symbolTo ml-1 perpetual-txt fw-5">&nbsp;{{
              $t("contract.swap") }}</span><span class="margin-trade-txt ml-1"
            v-if="(tab == '-1' || row.itype == -1) && $store.state.symbleinfos?.length > 0">{{
              $store.state.symbleinfos.filter(item => item.id == row.ctid)[0].borrowmultiple }}X</span>
        </p>
    ...
    <div slot="high" slot-scope="text,row">
      <p class="mb-0">{{ global_get_tofixed(row.h, global_get_decimal(row.symble).p) }}</p>
    </div>
    <div slot="low" slot-scope="text,row">
      <p class="mb-0">{{ global_get_tofixed(row.l, global_get_decimal(row.symble).p) }}</p>
    </div>
    <div slot="vol" slot-scope="text,row">
      <p class="mb-0">{{ numFormatter(global_get_tofixed(row.v, global_get_decimal(row.symble).p)) }}</p>
    </div>
    ...
    <div slot="change" slot-scope="text,row">
      <span :class="row.c - row.o < 0 ? 'red-active' : 'up-dark'">{{ getlimitprice(row) }}%<img
          :src="limitPrice(row.c, row.o) < 0 ? '/images/red-up-arrow.svg' : '/images/green-blue-up-arrow.svg'" height="12"
          width="auto" class="ml-1 mb-1" /> </span>
    </div>
    <span slot="action" slot-scope="text,row">
      <nuxt-link :to="`/market/detail?currency=${rowSymbel(row.symble)}&currency1=${rowSymbel1(row.symble)}`"> <span
          class="pointer second-txt">Details</span></nuxt-link>
    </span>
  </a-table>
</template>
...
export default {
  ...
  data() {
    ...
    return {
      columns: [
        ...
        {
          title: this.$t("tableskeys.tk3"),
          dataIndex: 'high',
          key: 'high',
          scopedSlots: { customRender: 'high' },
          width: 120,
        },
        {
          title: this.$t("tableskeys.tk4"),
          dataIndex: 'low',
          key: 'low',
      ...
      pagination: {
        ...
        pageSize: _this.pageSize, // 每页条数，所有页设置统一的条数
        total: _this.pageTotal //数据总数
      }
    }
  },
  watch: {
    'marketdata': function (val) {
      // console.log('marketdata::watch', val);
      if (val.length > 0) {
        this.loading = false;
        this.updateChartData()
      }
    },
    'pageTotal': function (val) {
  ...
  methods: {
    ...
    handleRowClick(row) {
      goDeal('' + row.symble + '', row.itype == 1 ? 'contract' : (row.itype == -1) ? 'margin' : 'spot');
    },
    goDeal(coinid, ctype) {
    ...
    pageChange(e) {
      ...
      this.$emit('change-size', e.current)
    },
    resetPageIndex() {
    ...
    collect(val) {
      ...
      url = isadd ? 'user_self_selection_add' : 'user_self_selection_delete';
    ...
    async getArticleInfo(row) {
      if (row.expanded) {
        ...
        if (!this.coinInfos[symble])
          this.getCoinInfo(symble);
      }
    },
    rowSymbel(symble) {
      const currency = symble.split('/')
      return currency[0]
    },
    rowSymbel1(symble) {
      const currency = symble.split('/')
  ...
  mounted() {
    // console.log('marketdata::mounted', this.marketdata);
    this.updateChartData()
  },
}
</script>
