<template>
  <div class="baseselect">
    ...
    <div v-show='ulShow' class="skill">
      ...
      <ul v-hex-loading="loading" class='por'>
        <li v-if="loading"></li>
        ...
        <template v-if='!statusa'>
          <li v-for="(item , i) in getCurrencyList" :key="i">
            <div class='addressState clearfix' @click='changeVal(item)'>
              ...
              <span class="addressState_tip">{{item.ctype == "btc" ? "omni" : item.ctype == "eth"?"erc20":item.ctype == "trx"?'trc20':'' }} &nbsp;&nbsp;&nbsp;&nbsp; {{item.remark}}</span>
            </div>
...
<style lang='scss' scoped>
  ...
  .baseselect {
    ...
    .ipt_hover {
      ...
      cursor: pointer;
      background: white url('./../../static/images/an_select_down.png') no-repeat 97% 15px;
    }
    .tip {
      position: absolute;
      right: 30px;
    ...
    .hovers {
      background: white url('./../../static/images/an_select_up.png') no-repeat 97% 15px;
    }
    .skill {
      position: absolute;
      width: 100%;
      ...
      .skill-ipt {
        cursor: pointer;
        &:before {
          background: url('./../../static/images/icon_setting.png') no-repeat;
          content: '';
