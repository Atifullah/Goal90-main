...
import { api, blogChannelId, helpCenterId, bitnasdaqNewsChannelId } from "~/plugins/constants"
...
export default {
  ...
  getArticleChildren(state, getters, rootState, rootGetters) {
    ...
    const channels = children.map(item => {
      ...
      let url = routeList.helpCenterChannelDetail + '/' + parents[parents.length - 1]
  ...
  getLatestArticles(state, getters, rootState, rootGetters) {
    ...
    const updatedArr = articles.map(obj => {
      ...
      if (shortdescription == '' || shortdescription == null || shortdescription == " ") {
      ...
      let url = routeList.helpCenterChannelDetail + '/' + mainChannelId + '?mainChannel=' + articleType + '&subChannel=' + articleType + '&subItemTitle=' + obj.subtitle
      ...
      const subChannelRdirectTo = routeList.helpCenterChannelDetail + '/' + mainChannelId + '?channelId=' + articleType
  ...
  getNewsSubItems(state, getters, rootState, rootGetters) {
    ...
    const updatedArr = items.map(obj => {
      ...
      const mainChannel = parents.length > 1 ? parents[parents.length - 2] : ''
      const subChannel = parents[parents.length - 1]
 ...
 getSearchResults: (state, getters, rootState, rootGetters) => {
    ...
    return updatedArr.sort((a, b) => new Date(b.updatetime).getTime() - new Date(a.updatetime).getTime());
  ...
  getSubItem(state) {
    ...
    try {
      if (subTitle[subTitle.length - 1] == 'faqs' && shortDescription) { //for faqs subtitle will contain _faqs at the end i.e titleof_faqs
  ...
  getArticleComments(state) {
    ...
    const array = comments.map(comment => {
      return {
        ...comment,
        createTime:moment.tz(comment.createtime, 'Etc/GMT').local().fromNow()
      }
    })
  ...
  getAnnouncement(state) {
    const currentChannel = state.announcements
    return currentChannel
  },
  ...
  getFooterData(state, getters, rootState, rootGetters) {
     ...
     const routeList = rootState.route
    const apiChannel = state.footerData.find((channel) => channel.id == api.channel)
    let apiObject = {}

    if (apiChannel) {
      apiObject = apiChannel.children.find((channel) => channel.channelname.toLowerCase().includes('api'))
   

      if (apiObject) {
        apiObject.subTitle = apiObject.description

        const staticUrl = routeList.helpCenterChannelDetail + '/' + helpCenterId + '?mainChannel=' + api.channel + '&subItemTitle=' + apiObject.description
        apiObject.imageurl = apiObject.imageurl ? apiObject.imageurl : staticUrl
