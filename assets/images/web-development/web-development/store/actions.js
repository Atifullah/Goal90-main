...
export default {
  async nuxtServerInit({ dispatch, commit, state }, { app, req, error }) {
    try {
      const cookie = Cookie.parse(req.headers.cookie ? req.headers.cookie : ''),
        hex_lang = cookie[state.hex_lang.type] ? cookie[state.hex_lang.type] : state.hex_lang.default,
        hex_exchange = cookie[state.hex_client_exchange.type] ? cookie[state.hex_client_exchange.type] : state.hex_client_exchange.default,
        hex_key = cookie[state.hex_key.type] ? cookie[state.hex_key.type] : '',
        hex_uid = cookie[state.hex_uid.type] ? cookie[state.hex_uid.type] : '',
        hex_background = cookie[state.hex_client_background.type] ? cookie[state.hex_client_background.type] : state.hex_client_background.default,
        hex_tradeview = cookie['tradeView'] ? cookie['tradeView'] : 'std',
        hex_token = cookie[state.hex_token.type] ? cookie[state.hex_token.type] : '';
      //设置默认语言
  ...
  gettoken({ commit }, params) {
    return this.$axios.post('/comapi/v1.0/com/token.get', params)
  },
...
trading_fundingrate({ commit }, params, config) {
  ...
  com_symbleinfos_get({ commit }, params, config) {
    return this.$axios.post("/comapi/v1.0/com/symbleinfos.get", params, config);
  },
