<template>
    <div class="trade-detail pt-3">
      <a-card :bordared="false" class="deposit-detail-card" :title="currencySelected+' '+$t('wallet.moneyRecord')">
        <div slot="extra">
          <a-row :gutter="16">
            <a-col :lg="24">
        ...
        <a-table
          ...
          rowKey="id"
        >
            <div slot="chargeTime" slot-scope="text,row" >
          ...
          <div slot="action" slot-scope="text,row">
            <a-row :gutter="16">
              ...
              <a-col :lg="8">
                <a-button class="withdraw-btn" block>{{St('deposit.dc7')}}</a-button>
              </a-col>
              <a-col :lg="8" aling="end">
                <a-button class="transaction-btn" block>{{St('deposit.dc8')}}</a-button>
              </a-col>
            </a-row>
          </div>
          <span slot="status">
            <a-button class="deposit-success fw-5">{{St('deposit.dc9')}}</a-button>
          </span>
        </a-table>
      </a-card>
    </div>
    ...
    data() {
      return {
        tradeTableColumns:[
          ...
          {
            title: this.$t('tableskeys.vt22'),
            dataIndex: 'address',
            width: 310,
          },
          {
            title: this.$t('tableskeys.vt23'),
            dataIndex: 'amount',
            width: 160,
          },
          {
