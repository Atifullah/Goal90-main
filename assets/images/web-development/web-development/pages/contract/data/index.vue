<template>
    <div class="hex-flex" style="background: #f5f5f8;">
        <div class="w1200 clearfix" style="margin: 80px auto;">
            <div class="fundrate_title">
                <span>{{ tabname }}</span>
                <div style="float: right;">
                    <a-select v-model="ctid" :placeholder="$t('placeholders.plh69')" @change="selectChanged" style="width: 280px">
            ...
            <div class="nav-container">
                <div class="tab">
                    <span :class="tab == 1 ? 'current' : ''" @click="changeTab(1)">{{ $t('contract.index_data') }}</span>
                     
                    <span :class="tab == 3 ? 'current' : ''" @click="changeTab(3)">{{ $t('contract.fund_data') }}</span>
                </div>
            </div>
            <hex-indexprice v-if="tab == 1" :ctid="ctid" :symblename="symblename"></hex-indexprice>
         
            <hex-fundrate v-if="tab == 3" :ctid="ctid" :symblename="symblename"></hex-fundrate>
        </div>
    </div>
</template>

<script>
import fundrate from '@/components/contract/fundrate'
import indexprice from '@/components/contract/indexprice'
 
export default {
    name: "contract_data_info",
    components: {
        'hex-fundrate': fundrate,
        'hex-indexprice': indexprice,
    ...
    methods: {
        ...
        selectChanged(value) {
            if (this.$store.getters.get_contractType == 1) {
                window.location.href = "/contract/data?ctid=" + value + '&contractType=1';
            } else {
